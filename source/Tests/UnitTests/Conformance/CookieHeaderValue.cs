...
namespace Thinktecture.IdentityServer.Tests.Conformance {
    public class CookieHeaderValue {
        ...
        public static bool TryParse(string input, out CookieHeaderValue parsedValue){
            var segmentSeparator = new[] {';'};

            parsedValue = null;
            if (string.IsNullOrEmpty(input)) {
                return false;
            }
            var strArray = input.Split(segmentSeparator);
            var instance = new CookieHeaderValue();
            if (strArray.Any(str => !ParseCookieSegment(instance, str))) {
                return false;
            }
        ...
        private static bool TryParseDate(string input, out DateTimeOffset result){
            ...
            return DateTimeOffset.TryParseExact(input, dateFormats,
        ...
        private static bool ParseCookieSegment(CookieHeaderValue instance, string segment){
            var nameValueSeparator = new[] {'='};

            if (string.IsNullOrWhiteSpace(segment)) {
                return true;
            }
            var nameValuePair = segment.Split(nameValueSeparator, 2);
            if ((nameValuePair.Length < 1) || string.IsNullOrWhiteSpace(nameValuePair[0])) {
                return false;
            }
            var a = nameValuePair[0].Trim();
            ...
            var segmentValue = GetSegmentValue(nameValuePair, null);
            try {
                var values = new FormDataCollection(segmentValue).ReadAsNameValueCollection();
                var item = new CookieState(a, values);
        ...
        private static string GetSegmentValue(IReadOnlyList<string> nameValuePair, string defaultValue){
