...
namespace Owin
{
    internal class SameSiteChunkingCookieManager : ICookieManager
    {
        ...
        public SameSiteChunkingCookieManager()
        {
            ChunkSize = 4090;
                ...
                for (int chunkId = 1; chunkId <= chunksCount; chunkId++)
                {
                    ...
                    if (chunk == null)
                    {
                        if (ThrowForPartialCookies)
                        {
                            ...
                            throw new FormatException("Resources.Exception_ImcompleteChunkedCookie");
                        }
        ...
        public void AppendResponseCookie(IOwinContext context, string key, string value, Microsoft.Owin.CookieOptions options)
        {
            ...
            else if (ChunkSize.Value < prefix.Length + suffix.Length + (quoted ? 2 : 0) + 10)
            {
                ...
                throw new InvalidOperationException("Exception_CookieLimitTooSmall");
            }
            else
            {
        ...
        private string GetSameSiteSuffix(IOwinContext context)
        {
            var options = context.ResolveDependency<IdentityServerOptions>();

            if (options.AuthenticationOptions.CookieOptions.SuppressSameSiteNoneCookiesCallback != null &&
                options.AuthenticationOptions.CookieOptions.SuppressSameSiteNoneCookiesCallback.Invoke(context.Environment))
            {
                return null;
            }
